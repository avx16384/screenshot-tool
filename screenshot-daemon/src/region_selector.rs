/// Standalone region selector + annotation tool.
/// Spawned by screenshot-daemon with: region-selector --output <path>
///
...
fn main() -> eframe::Result<()> {
    ...
    // Parse --background <path> if provided (pre-captured by screenshot-daemon)
    let bg_path = if let Some(idx) = args.iter().position(|a| a == "--background") {
