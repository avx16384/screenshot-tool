...
impl Recorder {
    pub fn new(display_server: &crate::detect::DisplayServer) -> Self {
        let output_path = new_output_path();
        if let Some(save_dir) = output_path.parent() {
    ...
    pub async fn start(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(save_dir) = self.output_path.parent() {
    ...
    pub async fn stop(&mut self) -> anyhow::Result<PathBuf> {
        ...
        self.paused = false;
        let metadata = std::fs::metadata(&self.output_path).map_err(|error| {
            anyhow::anyhow!(
                "recording output missing after stop: {}: {error}",
                self.output_path.display()
            )
        })?;
        if metadata.len() == 0 {
            anyhow::bail!(
                "recording output is empty after stop: {}",
                self.output_path.display()
            );
        }
...
fn capture_wayland_region(x: i32, y: i32, w: u32, h: u32) -> anyhow::Result<Vec<u8>> {
    ...
    let rgba_buf = img
        .as_rgba8()
        .ok_or_else(|| anyhow::anyhow!("failed to get rgba8 buffer"))?;
